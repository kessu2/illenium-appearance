...
local function setTattoos(ped, tattoos, style)
    ...
    for k in pairs(tattoos) do
        local zone = tattoos[k]
        if type(zone) == "table" and (not Config.AutomaticFade or k ~= "ZONE_HAIR") then
            for i = 1, #zone do
                local tattoo = zone[i]
                if type(tattoo) == "table" and tattoo.collection then
                    local tattooGender = isMale and tattoo.hashMale or tattoo.hashFemale
                    if tattooGender and tattooGender ~= "" then
...
local function setPreviewTattoo(ped, tattoos, tattoo)
    if not tattoo or not tattoo.collection or not (tattoo.hashMale or tattoo.hashFemale) then return end
    local isMale = client.getPedDecorationType() == "male"
    local tattooGender = isMale and tattoo.hashMale or tattoo.hashFemale
    if not tattooGender then return end
    ...
    if tattoos and type(tattoos) == "table" then
        for k in pairs(tattoos) do
            ...
            if type(zone) == "table" then
                for i = 1, #zone do
                    local aTattoo = zone[i]
                    if type(aTattoo) == "table" and aTattoo.name ~= tattoo.name and aTattoo.collection then
                        local aTattooGender = isMale and aTattoo.hashMale or aTattoo.hashFemale
                        if aTattooGender and aTattooGender ~= "" then
