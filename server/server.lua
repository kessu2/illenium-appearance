...
if Config.EnablePedMenu then
    if Config.PedMenuAll then
        -- Kaikki voivat käyttää /pedmenu omaan hahmoon, ei id-parametria
        lib.addCommand("pedmenu", {
            help = _L("commands.pedmenu.title"),
        }, function(source)
            TriggerClientEvent("illenium-appearance:client:openClothingShopMenu", source, true)
        end)
    else
        -- Vain admin-ryhmä, voi antaa toiselle: /pedmenu [id]
        lib.addCommand("pedmenu", {
            help = _L("commands.pedmenu.title"),
            restricted = Config.PedMenuGroup,
            params = {
                { name = "id", type = "playerId", help = _L("commands.pedmenu.targetId") or "Player ID", optional = true }
            }
        }, function(source, args)
            local target = (args.id and args.id ~= "") and tonumber(args.id) or source
            ...
            if not targetPlayer or targetPlayer == "" then
                TriggerClientEvent("ox_lib:notify", source, { type = "error", title = _L("commands.pedmenu.failure.title"), description = _L("commands.pedmenu.failure.description") })
                return
            end
